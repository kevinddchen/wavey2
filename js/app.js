...
const STOPS = [[20,80,200],[0,180,220],[240,200,0],[200,0,0]];
...
function buildHeatmapURL(data, tIdx) {
  ...
  for (let y = 0; y < ny; y++) {
    for (let x = 0; x < nx; x++) {
      ...
      if (h == null || isNaN(h)) { img.data[p + 3] = 0; continue; }
...
function generateDemoData() {
  const nx = 38, ny = 38, nt = 49;
  const lat_min = 36.50, lat_max = 36.88;
  const lon_min = -122.15, lon_max = -121.75;
  const t0 = new Date('2026-04-30T12:00:00Z');
  const times = Array.from({length: nt}, (_, i) =>
    new Date(t0.getTime() + i * 3.6e6).toISOString());

  const wave_height = [], wave_dir = [], water_level = [];
  for (let y = 0; y < ny; y++) {
    for (let x = 0; x < nx; x++) {
      ...
      for (let t = 0; t < nt; t++) {
        ...
        wd.push(Math.round(((315 + 20 * Math.sin(t * Math.PI / 24) + 8 * Math.sin(t * 0.4 + y * 0.2)) % 360 + 360) % 360));
        wl.push(+(0.65 * Math.sin(2 * Math.PI * t / 12.42) + 0.1 * Math.sin(t * Math.PI / 36)).toFixed(2));
      }
      wave_height.push(wh); wave_dir.push(wd); water_level.push(wl);
    }
  }
  ...
  return {
    ...
    wave_height, wave_dir, water_level,
    _demo: true,
  };
}
...
function makeChart(id, color, yLabel, yMin, yMax, tickCb) {
  const cfg = {
    ...
    options: {
      ...
      scales: {
        x: {
          ticks: { color: TICK_COLOR, maxTicksLimit: 6, maxRotation: 0, font: { size: 10 } },
...
const DIR_NAMES = {0:'N',45:'NE',90:'E',135:'SE',180:'S',225:'SW',270:'W',315:'NW',360:'N'};
...
function initCharts(times) {
  ...
  charts.dir    = makeChart('chart-dir',    '#e07a5f', '°', 0, 360,
    v => v % 45 === 0 ? `${DIR_NAMES[v] || ''} ${v}°` : null);
  ...
  Object.values(charts).forEach(c => {
    c.options.plugins.tooltip.callbacks.title = items => {
      ...
      const weekday = d.toLocaleString('en-US', { weekday: 'short', timeZone: 'America/Los_Angeles' });
  ...
  Object.values(charts).forEach(c => { c.data.labels = labels; c.update('none'); });
}

function updateCharts(data, gridIdx, tIdx) {
  charts.height.data.datasets[0].data = (data.wave_height[gridIdx] || []).map(v => v != null ? +(v * FEET_PER_METER).toFixed(2) : null);
  charts.dir.data.datasets[0].data    = data.wave_dir[gridIdx] || [];
  charts.tide.data.datasets[0].data   = (data.water_level || [])[gridIdx] || [];
  Object.values(charts).forEach(c => { c._currentIdx = tIdx; c.update('none'); });
}
...
async function init() {
  ...
  const map = L.map('map').setView(
    [(grid.lat_min + grid.lat_max) / 2, (grid.lon_min + grid.lon_max) / 2], 10
  );
  ...
  const mapBounds = [[grid.lat_min, grid.lon_min], [grid.lat_max, grid.lon_max]];
  const heatLayer = L.imageOverlay('', mapBounds, { opacity: 0.65, interactive: false }).addTo(map);
  ...
  sizeArrowCanvas();
  map.on('move zoom resize', () => { sizeArrowCanvas(); drawArrows(arrowTIdx); });
